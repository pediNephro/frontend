...
export class RejectionService {
  ...
  create(rejection: RejectionEpisodeDTO): Observable<RejectionEpisode> {
    const { transplantId, ...rest } = rejection as any;
    const body = transplantId ? { ...rest, transplant: { id: transplantId } } : rest;
    return this.http.post<RejectionEpisode>(this.apiUrl, body);
  }
  ...
  update(id: number, rejection: Partial<RejectionEpisodeDTO>): Observable<RejectionEpisode> {
    const { transplantId, ...rest } = rejection as any;
    const body = transplantId ? { ...rest, transplant: { id: transplantId } } : rest;
    return this.http.put<RejectionEpisode>(`${this.apiUrl}/${id}`, body);
  }
