...
export class BiopsyService {
  ...
  create(dto: BiopsyDTO): Observable<Biopsy> {
    ...
    return this.http.post<Biopsy>(this.apiUrl, body);
  }

  getAll(): Observable<Biopsy[]> {
    return this.http.get<Biopsy[]>(this.apiUrl);
  }
  ...
  update(id: number, dto: Partial<BiopsyDTO>): Observable<Biopsy> {
    const { transplantId, ...rest } = dto as any;
    const body = transplantId ? { ...rest, transplant: { id: transplantId } } : rest;
    return this.http.put<Biopsy>(`${this.apiUrl}/${id}`, body);
  }
  ...
  getByTransplantId(transplantId: number): Observable<Biopsy[]> {
  return this.http.get<Biopsy[]>(
