...
export class TransplantFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    // Get patientId from input, route params, or query params
    const routePatientId = this.route.snapshot.params['patientId'];
    const queryPatientId = this.route.snapshot.queryParams['patientId'];
    const effectivePatientId = this.patientId || (routePatientId ? +routePatientId : undefined) || (queryPatientId ? +queryPatientId : undefined);
    
    this.patientId = effectivePatientId;
    this.initForms();
  }
  ...
  saveTransplantInfo(): void {
    ...
    this.transplantService.create(transplantDTO).subscribe({
      next: (result) => {
        ...
        this.patientId = formValue.patientId; // Update patientId from form
        this.currentStep = 2;
      ...
      next: () => {
        this.transplantService.generateDefaultProtocol(this.createdTransplantId!).subscribe({
          next: () => {
            ...
            // Navigate to detail page with protocol tab active
            this.router.navigate(['/transplant', this.createdTransplantId], { 
              queryParams: { tab: 'protocol' } 
            });
          },
          error: () => {
