...
export class BiopsyListComponent implements OnInit {
  ...
  constructor(private biopsyService: BiopsyService, private route: ActivatedRoute) {}
  ...
  ngOnInit(): void {
    ...
    if (effectiveTransplantId) {
      this.biopsyService.getByTransplant(effectiveTransplantId).subscribe({
        next: (data) => { this.biopsies = data; this.isLoading = false; },
        error: () => { this.error = 'Failed to load biopsies.'; this.isLoading = false; }
      });
    } else {
      this.biopsyService.getAll().subscribe({
        next: (data) => { this.biopsies = data; this.isLoading = false; },
        error: () => { this.error = 'Failed to load biopsies.'; this.isLoading = false; }
      });
    }
